...
# Classes
##
class GSIExperimentRunner(ExperimentRunner):
...
if __name__ == '__main__':
   ...
   experiment_runner = GSIExperimentRunner(cfg)
   experiment_runner.set_log(log)
   parse_options(experiment_runner)

   if cfg.warm_init or cfg.cold_init :
      generate_gsi_workflow(cfg) # cold/warm-specific changes are handled internally
   ...
   if cfg.monitor_mode:
      ...
      if os.path.exists(cfg.rocoto_db_path):
         ...
         log.info('To restart the experiment, remove state file [%s]' 
                %os.path.relpath( os.path.realpath(cfg.rocoto_db_path),

...
MAXTRIES = '2' # ET doesn't autoconvert to string!
...
def get_wrf_num_cores():
   return str(392) # with nest, ask for 196
...
def get_wrf_walltime(fcst_time=3600):
   ...
   est = cfg.max_dom * int( ( 2400.0/3600) * fcst_time ) + 600 # add 10 minutes  for bdy update/warmup/data transfer/slowness
   return est
...
def get_enkf_num_cores():
   ...
   # The head node gets the h_xnobc for all workers, so it uses _lots_ of memory.
   # The next 3 also use a lot, possibly due to reading all obs of a given ob type 
   return "4:ppn=1+40:ppn=12"
   #return 1200 # was 196 for 9km # works much better for 30 members
...
def get_enkf_walltime():
   return str( 6600 + (cfg.num_members * 10 * 60) ) # TODO: Optimize
...
def add_wrf_ges_task(root):
   ...
   if cfg.da_type == 'enkf':
      # dependency depends on whethere additive inflation is being done
      if cfg.perform_enkf_additive_inflation:
         ...
         if not cfg.reuse_static_data:
            add_taskdep_node(taskdep_and, cfg, REAL_TASK_NAME, True)
   elif cfg.da_type == 'gsi':
      taskdep_and = ET.SubElement(dep, 'and')
   ...
   # TODO: Add dependency to REAL_ENSEMBLE_TASK_NAME
